...
public class ReplaceController implements Initializable {
    ...
    ReplaceController(Model model, TextArea textArea) {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        replaceField.setOnKeyPressed(new EventHandler<KeyEvent>() {
            @Override
            public void handle(KeyEvent event) {
                replaceButton.setDisable(false);
                if((event.getCode().equals(KeyCode.BACK_SPACE) ||event.getCode().equals(KeyCode.DELETE)) &&
                        (replaceField.getText().length() == 1 || replaceField.getText().length() == 0)){
                    replaceButton.setDisable(true);
                }
            }
        });
    ...
    private void findNext(){
        ...
        if(!check){
            int temp;
            if(up.isSelected()){
                temp = findNextWordUp(areaLetters, fieldLetters, chars, areaLetters.size());
            } else {
                temp = findNextWord(areaLetters, fieldLetters, chars, 0);
            }
            if(temp == -1 && !replaceAll){
            ...
            int temp;
            if(up.isSelected()){
                temp = findNextWordUp(areaLetters, fieldLetters, chars, position);
            } else {
    ...
    private int findNextWordUp(List<Character> areaLetters, List<Character> fieldLetters, List<Character> chars, int position){
        if(findField.getText().length() < 2){
    ...
    void replace(){
        ...
        System.out.println(textArea.getSelection().getStart());
    ...
    void replaceAll(){
        ...
        for(int i = 0; i <= wordCount; i++){
            if (textArea.getSelection().getLength() == 0 &&!checkTwo){
                break;
            }
            replace();
            checkTwo = false;
